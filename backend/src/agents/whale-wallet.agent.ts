import { loadWhaleWallets, WHALE_FETCH_DELAY_MS } from '../lib/main-agent.config';
import type {
  MarketRef,
  WhaleActivityEntry,
  WhaleActivityResult,
...
async function fetchWalletTrades(wallet: string, limit = TRADES_LIMIT): Promise<PolymarketTrade[]> {
  const url = `${TRADES_URL}?user=${wallet}&limit=${limit}`;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
    const res = await fetch(url, { signal: controller.signal });
    if (!res.ok) {
      console.warn(`[whale-agent] trades API ${res.status} for ${wallet}`);
      return [];
    }
    const body = await res.json();
    return Array.isArray(body) ? (body as PolymarketTrade[]) : [];
  } catch (err) {
    console.warn(`[whale-agent] trades fetch failed for ${wallet}: ${err}`);
    return [];
  } finally {
    clearTimeout(timer);
  }
...
export class WhaleWalletAgent {
  ...
  async track(request: WhaleTrackRequest): Promise<WhaleActivityResult> {
    ...
    if (entries.length === 0 && totalTrades > 0) {
      console.log(
        `[whale-wallet-agent] scanned ${totalTrades} trades across ${wallets.length} wallets, 0 matched ${markets.length} news markets`,
      );
    }
...
export const whaleWalletAgent = new WhaleWalletAgent();
