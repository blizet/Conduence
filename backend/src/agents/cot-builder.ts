...
import { MAIN_GRAPH_ID, MAIN_USER_NODE_ID } from '../lib/main-agent.config';
import { DecisionEventSchema, type DecisionEvent } from '../schemas/decision.schema';
import type { CorrelatedMarketsResult, GeminiTradeDecision } from './types';
...
export function buildCotDecision(
  ...
  const nodes: DecisionEvent['nodes'] = [
    { node_id: MAIN_USER_NODE_ID, node_type: 'user' },
  ...
  const edges: DecisionEvent['edges'] = [
    { source: MAIN_USER_NODE_ID, target: 'Polymarket' },
    {
      source: 'Polymarket',
      target: marketId,
      metadata: {
