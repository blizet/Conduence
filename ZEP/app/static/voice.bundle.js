...
var require_voice = __commonJS({
  "static/voice.js"() {
    ...
    var WEBRTC_ENDPOINT = "/api/offer";
    var configReady = document.body.dataset.configReady === "true";
    var voiceLog = document.getElementById("voice-log");
    var connectBtn = document.getElementById("voice-connect");
    var statusDot = document.getElementById("voice-dot");
    var statusLabel = document.getElementById("voice-status-label");
    var micMeter = document.getElementById("voice-meter-mic");
    var botMeter = document.getElementById("voice-meter-bot");
    var botAudio = document.getElementById("voice-bot-audio");
    var client = null;
    var isVoiceReady = false;
    var botMsgEl = null;
    var partialUserEl = null;
    function setVoiceState(state, label) {
      statusDot.className = `voice-dot voice-dot--${state}`;
      statusLabel.textContent = label;
    }
    function scrollVoiceLog() {
      if (voiceLog) voiceLog.scrollTop = voiceLog.scrollHeight;
    }
    function addVoiceMessage(role, content, { pending = false } = {}) {
      if (!voiceLog) return null;
      const intro = voiceLog.querySelector(".log__intro");
      if (intro) intro.remove();
      const row = document.createElement("div");
      row.className = `msg msg--${role}${pending ? " msg--pending" : ""}`;
    ...
    function attachBotAudio(track2) {
      if (!botAudio || !track2) return;
      botAudio.srcObject = new MediaStream([track2]);
      botAudio.play().catch(() => {
      });
    }
    async function ensureClient() {
      ...
      client = new $364c127d152b1085$export$8f7f86a77535f7a3({
        ...
        callbacks: {
          onTransportStateChanged: (state) => {
            isVoiceReady = state === "ready";
            if (state === "ready") {
              setVoiceState("connected", "Connected \u2014 speak naturally");
              connectBtn.textContent = "Disconnect";
              connectBtn.classList.add("voice-btn--active");
              connectBtn.disabled = false;
            } else if (state === "connecting" || state === "connected" || state === "initializing") {
              setVoiceState("connecting", "Connecting\u2026");
              connectBtn.disabled = true;
            } else if (state === "disconnecting") {
              setVoiceState("connecting", "Disconnecting\u2026");
              connectBtn.disabled = true;
            } else {
              isVoiceReady = false;
              setVoiceState("", "Ready to connect");
              connectBtn.textContent = "Start voice";
              connectBtn.classList.remove("voice-btn--active");
              connectBtn.disabled = !configReady;
            }
          },
          onUserTranscript: (data) => {
            if (!data?.text) return;
            if (data.final) {
              if (partialUserEl) {
                partialUserEl.textContent = data.text;
              ...
              if (!partialUserEl) {
                partialUserEl = addVoiceMessage("user", data.text, { pending: true });
              } else {
                partialUserEl.textContent = data.text;
              }
            }
          ...
          onBotOutput: (data) => {
            ...
            if (data.aggregated_by === "sentence" || data.aggregated_by === "word") {
              if (!botMsgEl) {
                botMsgEl = addVoiceMessage("assistant", data.text);
              } else {
                botMsgEl.textContent = botMsgEl.textContent ? `${botMsgEl.textContent} ${data.text}`.trim() : data.text;
              }
              scrollVoiceLog();
            }
          },
          onBotStartedSpeaking: () => {
            botMsgEl = null;
            setVoiceState("speaking", "Conduence is speaking");
          },
          onBotStoppedSpeaking: () => {
            botMsgEl = null;
            setVoiceState("connected", "Listening");
          },
          onUserStartedSpeaking: () => setVoiceState("listening", "Listening\u2026"),
          onUserStoppedSpeaking: () => setVoiceState("connected", "Processing\u2026"),
          onLocalAudioLevel: (level) => setMeter(micMeter, level),
          onRemoteAudioLevel: (level) => setMeter(botMeter, level),
          onTrackStarted: (track2, participant) => {
            if (track2?.kind === "audio" && participant && !participant.local) {
              attachBotAudio(track2);
            }
          },
          onError: (err) => {
            ...
            connectBtn.disabled = false;
            connectBtn.textContent = "Retry voice";
            connectBtn.classList.remove("voice-btn--active");
          }
        }
      });
    ...
    if (connectBtn) {
      ...
      connectBtn.addEventListener("click", async () => {
        try {
          ...
          setVoiceState("error", err.message || "Could not connect");
          connectBtn.disabled = false;
          connectBtn.textContent = "Retry voice";
        }
      });
